...
public class UnboundedKnapsack {
    public static void main(String[] args) {
        ...
        int ans=unboundedKnapsack(dp,A,B,n-1,C);
        System.out.println(ans);
    }

    private static int unboundedKnapsack(int [][]dp,int[] a, int[] b, int end, int remainingWeight) {
        if(end==0){
        ...
        if(dp[end][remainingWeight]!=-1)
            ...
            if (remainingWeight >= b[end])
                include = a[end] + unboundedKnapsack(dp, a, b, end , remainingWeight - b[end]);
            int exclude = unboundedKnapsack(dp, a, b, end-1, remainingWeight);
